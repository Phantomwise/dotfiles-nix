...
function download_video_w_sub {
    ...
    yt-dlp -f - -o "%(title)s [%(id)s] [%(format_id)s].%(ext)s" --write-subs --sub-langs "all" --impersonate="chrome:windows-10" --sleep-interval 20 --max-sleep-interval 40 --limit-rate 600K "$url" && \
